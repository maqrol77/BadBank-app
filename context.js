...
function BankForm(props, { chooseShowP }) {
    ...
    /* Validate that has email@gmail.com format */
    ...
    function validateLogin(fieldEmail, fieldPwd) {
        ...
        for (let i in usersArray) {
            ...
            if (fieldEmail === usersArray[i]['email'] && fieldPwd === usersArray[i]['password']) {
                ...
                props.chooseStatusP('Incorrect password. Try again.');
                setTimeout(() => props.chooseStatusP(''), 5000);
            }
        }
    }
    ...
    /* Validate that money is a number */
    ...
    /* Validate that the deposit is a positive number greater than 0 */
    ...
    /* Validate that the withdrawal does NOT exceed the current balance. */
    ...
    /* Clear deposit form after adding a new deposit */
    function clearFormD() {
        setDeposit('');
        setShow(true)
        props.chooseShowP(true)
    ...
    /* Delete Withdrawal Form after Adding New Withdrawal */
    function clearFormW() {
        setWithdraw('');
        setShow(true)
        props.chooseShowP(true)
